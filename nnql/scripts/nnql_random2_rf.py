...
def frontier():
    ...
    rosparam.set_param("/move_base/base_global_planner",
...
def get_reward(occupancy_init, occupancy, distance):
    ...
    rospy.loginfo('init:%d, occupancy:%d, distance:%f, reward:%f',
                  occupancy_init, occupancy, distance, reward)
    return reward
...
if __name__ == '__main__':
    ...
    status_csv = 'NNQL_random2_rf/groundtruth.csv'
    #scan_csv = 'scan' + args[1] + '.csv'
    f_status = open(status_csv, 'w')
    #f_scan = open(scan_csv,'w')
    result_csv = 'NNQL_random2_rf/result.csv'
    f_result = open(result_csv, 'w')
    qdata_path = 'NNQL_random2_rf/Qdatabase'
    episode = 1  # 901
    max_episode = 600
    step = 1
    ...
    while (episode < max_episode+1):
        ...
        while (step < 5):
            ...
            if not (episode < 25):
                q_average, knn_list = NNQL.knn(
                    now_state, use_Qdatabase)  # knn,now_state=クラス固有ベクトル
                action_maxIndex = [i for i, x in enumerate(
                    q_average) if x == max(q_average)]
            #  action_index = np.random.choice(action_maxIndex) # action_index = max(q_avg)
            #  action = action_list[action_index]
            else:
                q_average = list([0]*3)
            # action_index = 2 * np.random.randint(0, 2) #frontier,random
                knn_list = None

            action_index = 2 * np.random.randint(0, 2)  # frontier,random
            action = action_list[action_index]
            Qdatabase = NNQL.Q_data_add(
                now_state, q_average, Qdatabase)  # qデータベース追加
            ...
            env_list.append(
                [now_state, next_state, action_index, knn_list, q_average])  # 結果保存
        ...
        result_episode = str(reward)+','+str(occupancy_init) + \
            ','+str(occupancy)+','+str(distance)+'\n'
        ...
        for env in env_list:
            ...
            knn_list_all = env[3]
            q_list = env[4]
            Qdatabase = NNQL.q_learning(
                state, state_next, do_action, reward, knn_list_all, q_list, Qdatabase)  # q値更新
